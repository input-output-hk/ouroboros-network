...
instance (Show peer) => Show (TraceChainSyncClientTag peer) where
    show (IntersectTag peer) = "IntersectTag " ++ show peer
    show (NextTag peer)      = "NextTag " ++ show peer
...
peerReqRspTracer tracer peer = do
    ...
    return $ transform $ matchTaggedObservations
        ...
        (\case
            (IntersectTag _) ->
                void . atomically . swapTMVar intersectState . Just
...
chainSyncClientPeerSender tracer n@Zero (SendMsgRequestNext stNext stAwait) = SenderEffect $ do
